...
template <typename T> struct BrickInterpFact<T, 1> {
  static inline T compute(T r) {
    return (T)(0.5*(1+r));
  }
  static inline T compute_deriv_1st(T r) {
...
template <typename T> struct BrickInterpFact<T, 0> {
  static inline T compute(T r) {
    return (T)(1-r*r);
  }
  static inline T compute_deriv_1st(T r) {
...
template <unsigned int I> struct BrickInterpCoord {
  const static int R0;
  const static int R1;
  const static int R2;
};
...
#define FORM_MACRO_BrickInterpCoord(i, x, y, z) \
  template <> struct BrickInterpCoord<i> { \
...
template <typename T, unsigned int I> struct BrickInterpMono {
...
template <typename T, unsigned int I, unsigned int N> struct BrickInterpPoly {
  static inline T compute(T r0, T r1, T r2);
  static inline T compute_d1_r0(T r0, T r1, T r2);
  static inline T compute_d1_r1(T r0, T r1, T r2);
  static inline T compute_d1_r2(T r0, T r1, T r2);
};
template <typename T, unsigned int I> struct BrickInterpPoly<T, I, 8> {
  static inline T compute(T r0, T r1, T r2) {
...
#define FORM_MACRO_BrickInterpPoly_N20_LOWER(i, j, k, l) \
  template <typename T> struct BrickInterpPoly<T, i, 20> { \
    static inline T compute(T r0, T r1, T r2) { \
