...
class CLS(object):
     ...
     @staticmethod
     def get_handle(name):
         if name not in CLS._cls_reg:
            raise NameError("class " + name + " has been not registered")
         return CLS._cls_reg[name]

     @staticmethod
     def register_class(name):
        def _wrapper(_cls):
            _cls.name = name
            if name in CLS._cls_reg:
                raise NameError("class " + name + " has been registered")
            CLS._cls_reg[name] = _cls
            return _cls
        return _wrapper
